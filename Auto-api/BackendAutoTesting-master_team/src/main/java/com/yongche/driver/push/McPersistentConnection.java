...
import com.yongche.driver.common.ClientException;
...
import com.yongche.driver.common.GroundhogClient;
import com.yongche.driver.common.ManagerClient;
...
import com.yongche.driver.message.GroundhogMessageException;
import com.yongche.driver.message.PushRequestMessage;
import com.yongche.driver.message.RpcRequestMessage;
import com.yongche.driver.message.RpcResponseMessage;
import com.yongche.driver.message.SubscribeRequestMessage;
import com.yongche.driver.message.UnsubscribeRequestMessage;
...
public class McPersistentConnection extends ActionHandler {
    ...
    protected void handleIntent(Intent i) throws PushConnectionException, MessageException, IOException, InterruptedException, JSONException {
        ...
        if (action.equals(PushService.ACTION_INIT)) {
            ...
            JSONArray managerHosts = JSONArray.fromObject(bdl.getStringArrayList("mc_manager_hosts"));
    ...
    private boolean needLaunch(Bundle bdl, McSessionInfo mcSession)
    {
        ...
        JSONArray hosts = JSONArray.fromObject(managerHosts);
    ...
    public boolean retrieveWorkerInfo() throws MessageException, IOException, InterruptedException, PushConnectionException, JSONException {
        ...
        if (mcSession.managerHosts == "") {
            ...
            managerHosts = JSONArray.fromObject(mcSession.managerHosts);
        }
        ...
        //        }
                ...
                try {
                    ...
                    e.printStackTrace();
                    managerConn.close();
                    sleep(CommonConfig.MANAGER_RETRY_INTERVAL);
                }
            }
        ...
        if (!managerConn.isConnected()) {
            throw new PushConnectionException("Push Manager Host can't be connected, manager hosts count is " + managerHosts.size());
        }
        ...
        try {
	        ...
	        if (PushService.subscribeEnabled) {
	            ...
	            {
	                workerChanged = true;
	                Logger.info(this.getClass().getName(), "client in different worker");
	            }
	        }
    ...
    public void makeWorkerConnection() throws IOException, MessageException, InterruptedException, PushConnectionException {
        ...
        for(int i = 0; i < CommonConfig.MAX_RETRY_WORKER_TIMES; ++i) {
            try {
                ...
                e.printStackTrace();
                workerConn.close();
                sleep(CommonConfig.WORKER_RETRY_INTERVAL);
            }
        }
    ...
    public void subscribe(long driverId, long orderId, short messageType, long retryTimeout)
        ...
        try {
            if (this.retrieveWorkerInfo()) {
                if (this.isConnected()) {
                    Logger.info(this.getClass().getName(), "Close connection due to "
                            + "client in different worker");
                    this.close();
                }
            } else {
                Logger.info(this.getClass().getName(), "passenger and client are in same worker");
                if (this.isConnected()) {
            ...
            Logger.info(this.getClass().getName(), "client auth failed");
    ...
    public void printCurrentThreadStackTrace()
    {
        ...
        Logger.info(this.getClass().getName(), "Current Thread Id: " + currentThread().getId());
        for (StackTraceElement i: currentThread().getStackTrace()) {

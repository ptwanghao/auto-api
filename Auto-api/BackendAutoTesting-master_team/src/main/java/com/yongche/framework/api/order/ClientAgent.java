...
public class ClientAgent {
    ...
    private static final String MODULE_DECISION_DRIVER = "order/decisiondriver";
    private static final String MODULE_ACCEPT_CAR = "order/acceptcar";
    ...
    public static boolean decisionDriver(String order_id,String driver_id){
        //请求decisionDriver接口的url "http://testing.client-agent.yongche.org/order/decisiondriver";
        String host = ConfigUtil.getValue(ConfigConst.HOST,"CLIENT_AGENT");
        String url = String.format("https://%s/%s",host,MODULE_DECISION_DRIVER);
        ...
        //AUTHORIZATION与userId是一一对应的，目前尚无法实时获取，暂时用user_id=26016450的，即被决策的订单下单者必须为此用户。
        headerMap.put("Authorization", ConfigUtil.getValue(ConfigConst.RIDER_INFO,"access_token"));
    ...
    public static String getAcceptCar(String order_id,String user_id){
        //请求acceptcar接口的url  "http://testing.client-agent.yongche.org/order/acceptcar";
        String host = ConfigUtil.getValue(ConfigConst.HOST,"CLIENT_AGENT");
        String url = String.format("http://%s/%s",host,MODULE_ACCEPT_CAR);
        //构造参数
        List<NameValuePair> formParameters = new ArrayList<>();
        formParameters.add(new BasicNameValuePair("order_id",order_id));

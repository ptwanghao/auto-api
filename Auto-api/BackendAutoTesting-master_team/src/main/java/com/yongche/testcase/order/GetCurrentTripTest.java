...
//        MapUtil.put(inputParameterMap,"car_type","豪华车型");
    ...
    public int checkOrder(String new_order_id,boolean include){
        RiderInfo rider = CommonUtil.getRider(RiderCategoryConst.FIRST_USER);
        Map<String,TestParameter> inputParameterMap = MapUtil.getParameterMap("user_id",rider.getUserId());
        MapUtil.put(inputParameterMap,"cellphone", rider.getCellphone());

        //查询订单，获得返回结果
        Map<String,TestParameter> result = CaseExecutor.runTestCase("order_getCurrentTrip.xml",inputParameterMap);
